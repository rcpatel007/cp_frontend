...
export class EditOwnerComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      this.updatechartererForm = this._formBuilder.group({
          ...
          businessPhone: ['', Validators.pattern("[A-Za-z0-9]+")],
          firstName: ['', Validators.pattern("[A-Za-z0-9]+")],
          lastName: ['', Validators.pattern("[A-Za-z0-9]+")],
          mobileNo: ['', Validators.pattern("[0-9]+")],
      
      });
      this.returnUrl = this.route.snapshot.queryParams['returnUrl'] || '/apps/owner-management';
      // this.roleList();
      this.companyList();
      this.roleAccessList(); 
     ...
     // console.log('add user');
      ...
      if (this.updatechartererForm.invalid) {
          ...
          try {
              ...
              this.http.post(`${config.baseUrl}/OwnerEdit`, req, headerOptions).subscribe(
                  res => {
                      console.log(res);
  ...
  brokererdDetail(id): void {
      ...
      try {
          this._userService.OwnerDetails(requestData)
          .pipe(first())
          .subscribe((res) => {
